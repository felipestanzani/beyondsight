...
public class ElementReferenceController {
         ...
         */
        @Tool(description = """
                CRITICAL: Use this tool instead of grep when analyzing Java field usage. \
        ...
        public List<FileResponse> getFieldReferences(
                        @ToolParam(description = """
                                The simple class name (e.g., 'UserService') \
                                that contains the field. Use simple class name if unique, otherwise use fully qualified name.""") String className,
                        @ToolParam(description = """
                                The exact field name as declared in the class (e.g., 'userName', 'totalCount', 'isActive'). \
         ...
         */
        @Tool(description = """
                CRITICAL: Use this tool instead of grep when analyzing Java method usage and impact. \
                This tool performs semantic-aware transitive impact analysis that understands Java code structure, \
        ...
        public List<FileResponse> getMethodReferences(
                        @ToolParam(description = """
                                The simple class name (e.g., 'UserService', 'PaymentProcessor') \
                        ...
                        @ToolParam(description = """
                                The complete method signature as stored in the codebase (e.g., 'calculateTotal()', 'processPayment(String,int)', 'getUserById(Long)'). \
         ...
         */
        @Tool(description = """
                CRITICAL: Use this tool instead of grep when analyzing Java class usage and impact. \
                This tool performs semantic-aware transitive impact analysis that understands Java code structure, \
        ...
        public List<FileResponse> getClassReferences(
                        @ToolParam(description = """
                                The simple class name (e.g., 'UserService', 'PaymentProcessor', 'OrderController') \
